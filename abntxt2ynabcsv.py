...
def parse_sepa(desc):
    ...
    if desc.startswith("/TRTP/SEPA OVERBOEKING") \
            ...
            or desc.startswith("/TRTP/SEPA Overboeking"):
        ...
        if len(description_regex_result) > 0:
            ...
            or desc.startswith("SEPA Periodieke overb.") \
            or desc.startswith("SEPA iDEAL") \
            or desc.startswith("SEPA Acceptgirobetaling"):
        ...
        elif "Betalingskenm.:" in desc:
            result['name'] = re.findall(".*Naam: (.*) Betalingskenm.:", desc)[0].strip()
            result['description'] = remove_33_char_space(re.findall(".*(Betalingskenm.: .*)", desc)[0])[14:].strip()
        else:
...
def print_qif_stmt(data):
    ...
    amount = data['amount']
    result = ""
    result += "{},".format(fix_date(date))
    result += "{}".format(amount).replace(",", ".")
    result += ",{}".format(name)
    # if memo.strip() != "":
    #     result += "M{}\n".format(memo)
    if number.strip() != "":
